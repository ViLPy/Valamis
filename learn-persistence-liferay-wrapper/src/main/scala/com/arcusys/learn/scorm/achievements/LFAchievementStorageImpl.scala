...
trait LFAchievementStorageImpl extends KeyedEntityStorage[Achievement] {
  ...
  private def convert(lfAchievement: LFAchievement): Achievement = Achievement(
    ...
    new DateTime(lfAchievement.getCreationDate)
  )
  ...
  def createAndGetID(entity: Achievement, parameters: (String, Any)*): Int = LFAchievementLocalServiceUtil.createLFAchievement(entity.title, entity.description, entity.logo, entity.startDate.toDate).getId.toInt
  ...
  def modify(entity: Achievement, parameters: (String, Any)*) {
    ...
    lfEntity.setCreationDate(entity.startDate.toDate)
