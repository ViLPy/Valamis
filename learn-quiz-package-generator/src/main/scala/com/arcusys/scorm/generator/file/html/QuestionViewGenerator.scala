  ...
  def getHTMLByQuestionId(question: Question[Answer], contextPath: String = "") = {
    question match {
      case choiceQuestion: ChoiceQuestion =>
        ...
        val correctAnswers = json(choiceQuestion.answers.filter(_.isCorrect).map(x => prepareString(x.text))).get
      ...
      case numericQuestion: NumericQuestion =>
        val answers = json(numericQuestion.answers.map(answer => Map("from" -> answer.notLessThan, "to" -> answer.notGreaterThan))).get
      ...
      case categorizationQuestion: CategorizationQuestion =>
        val answerJSON = json(categorizationQuestion.answers.map(answer => Map("text" -> prepareString(answer.text), "matchingText" -> answer.answerCategoryText.map(prepareString)))).get
