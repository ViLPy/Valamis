...
import com.arcusys.valamis.quiz.storage.{ QuizTreeStorage, QuizQuestionStorage, QuizQuestionCategoryStorage, QuizStorage }
import com.escalatesoft.subcut.inject.{ Injectable, BindingModule }

class QuizServiceImpl(implicit val bindingModule: BindingModule) extends Injectable with QuizService
    with QuizQuestionServiceImpl with QuizCategoryServiceImpl {

  protected def fileService = inject[FileService]
    ...
    def cloneQuestion(quizQuestion: QuizQuestion, categoryId: Option[Int]): Unit = {
      quizQuestion match {
        ...
        case q: QuestionBankQuizQuestion =>
          val question = createQuestionFromQuestionBank(newQuiz.id, categoryId, q.question.id)
          updateQuestionFromQuestionBank(question.id, q.title.getOrElse(""), q.autoShowAnswer)
