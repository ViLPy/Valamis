...
class Attribute(elem: Option[Elem], namespace: Option[String], name: String) {
  ...
  def required(string: StringValue): String =
    if (elem.isEmpty) throw new RuntimeException("Parent element does not exist for attribute '" + name + "'")
    else if (attr.length == 0) throw new RuntimeException("<" + elem.get.label + "> element does not contain '" + name + "' attribute")
    else attr.text
...
class Child(elem: Elem, namespace: Option[String], name: String) {
  def required(ignore: ElemValue): Elem =
    optional(ignore).getOrElse(throw new RuntimeException("<" + elem.label + "> element does not contain <" + name + "> element"))
  ...
  def optional(ignore: ElemValue): Option[Elem] = {
    ...
    if (childElements.size > 1) throw new RuntimeException("<" + elem.label + "> element contains more than one <" + name + "> element")
