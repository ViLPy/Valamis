...
class GeneratedQuizIncorrectPass (_driver:WebDriver) extends Suite with FlatSpec with ShouldMatchers with UITestBase with LoginSupport {
  val driver = _driver
  ...
  private def processEssay() {
    ...
    driver.findElement(id("scormAnswer")).sendKeys("It's not ok!")
  ...
  private def processShortCaseInSensitive() {
    ...
    driver.findElement(id("scormAnswer")).sendKeys("aa")
  ...
  private def processNumeric() {
    ...
    driver.findElement(id("scormAnswer")).sendKeys("105")
    driver.findElement(tagName("button")).click()
    driver.switchTo().defaultContent()
    wait(3)
  }
