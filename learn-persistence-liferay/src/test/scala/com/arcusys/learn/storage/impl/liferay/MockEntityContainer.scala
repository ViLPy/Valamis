 ...
 * Date: 12.3.2013
 */
trait MockKeyedEntityContainer[A <: BaseLocalService, B <: BaseModel[B]] extends MockEntityContainer[A, B] {
...
trait MockEntityContainer[A <: BaseLocalService, B <: BaseModel[B]] extends Mockito {
  private val _internalStorage = new ThreadLocal[mutable.ConcurrentMap[Long, B]]
  val mockBeanLocator = Option(PortletBeanLocatorUtil.getBeanLocator(ClpSerializer.getServletContextName)).getOrElse(mock[BeanLocator])

  PortletBeanLocatorUtil.setBeanLocator(ClpSerializer.getServletContextName, mockBeanLocator)
