...
trait LFCertificateStorageImpl extends KeyedEntityStorage[Certificate] {
  ...
  def create(entity: Certificate, parameters: (String, Any)*) {throw new UnsupportedOperationException }

  def delete(parameters: (String, Any)*) {
    parameters.find(_._1 == "id").map(_._2.asInstanceOf[Int]) foreach { id => {
