...
import com.arcusys.learn.persistence.liferay.service.{LFTincanManifestActivityLocalServiceUtil, LFTincanActivityLocalServiceUtil}
  ...
  def getAll(parameters: (String, Any)*) = {
    parameters match {
      case Seq(("packageID", id: Any)) =>
        LFTincanManifestActivityLocalServiceUtil.findByPackageID(getLong(id)).asScala.map(extract)
    }
  }

  private def extract(lfEntity: LFTincanManifestActivity)={
...
//    LFTincanActivityLocalServiceUtil.updateLFTincanActivity(lfEntity)
  ...
  def getByID(id: Int, parameters: (String, Any)*) = {
    Option(LFTincanManifestActivityLocalServiceUtil.getLFTincanManifestActivity(id)).map(extract)
  }

  def createAndGetID(entity: ManifestActivity, parameters: (String, Any)*) = {
    val newEntity = LFTincanManifestActivityLocalServiceUtil.createLFTincanManifestActivity()
    ...
    LFTincanManifestActivityLocalServiceUtil.addLFTincanManifestActivity(newEntity).getId.toInt
  }
