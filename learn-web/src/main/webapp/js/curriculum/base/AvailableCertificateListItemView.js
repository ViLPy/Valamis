...
AvailableCertificateListItemView = Backbone.View.extend({
    ...
    joinCertificate:function(){
        window.LearnAjax.post(Utils.getContextPath() + "services/certificating/users/addUser/" + this.model.id,
            { 'userID':jQuery("#studentID").val(),
              'companyID':jQuery("#companyID").val()},
            jQuery.proxy(function (){ this.trigger('membership-changed');  }, this));
        this.leave();
    },
