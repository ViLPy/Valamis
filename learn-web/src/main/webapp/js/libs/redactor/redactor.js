...
(function($){
    ...
    function Construct(el, options) {
        ...
        this.opts = $.extend({	
            ...
            image_upload: Utils.getContextPath() + 'services/upload/upload-image', 
            imageGetJson: Utils.getContextPath() + 'services/upload/get-images',
            createFolder: Utils.getContextPath() + 'services/upload/create-folder',
            
            imageUploadFunction: false, // callback function	
            file_upload: Utils.getContextPath() + 'services/upload/upload-file', 
    ...
    Construct.prototype = {
        ...
        {
            if ($.browser.mozilla) // probably some verions of Mozilla need this
            {
		try{
        ...
        showImage: function()
        {
            ...
            var handler = function()
            {
                ...
                {
                    var _this = this;
                    getImagesByPath(this.opts.imageGetJson,this.opts.createFolder,"",_this);
                    /*$.post(this.opts.imageGetJson, function(data) {
                    $.each(data, function(key, val)
                    {                 
                        if(val.isDirectory=="false"){
                            var img = $('<div class="image-wrapper"><img class="image"src="' + val.thumb + '" rel="' + val.image + '"><div class="text-wrapper">'+val.name+'</div></div>');
                            img.click(function() { redactorActive.imageSetThumb($(this).attr('rel')); });	  		
                            $('#redactor_image_box').append(img);
                        }
                        else{
                            var folderImg = $('<div class="image-wrapper"><div class="folder"></div><div style="padding-top:5px;">'+val.name+'</div></div>');
                            folderImg.click(function() { redactorActive.imageSetThumb($(this).attr('rel')); });	  		
                            $('#redactor_image_box').append(folderImg);
                        }
            ...
            redactorActive = this;
            this.modalInit(RLANG.image, this.opts.path + 'plugins/image.html?r', 570, 520, handler);
        ...
        uploadForm : function(f, name)
        {
            if (this.uploadOptions.input)
            {
                ...
                var currentDir = this.uploadOptions.currentDir;
                var folderImg = $('<input type="text" name="currentDirectory" value="'+currentDir+'"></input>');
                var newElement = $(oldElement).clone();
                $(oldElement).attr('id', fileId);
                $(oldElement).before(newElement);
                $(oldElement).appendTo(this.form);
                folderImg.appendTo(this.form);
...
(function($){
    ...
    function Construct(el, options) {
        ...
        this.opts = $.extend({
            ...
            /*
			success_text: 'Загрузка у�?пешно завершена',
			progress_text: 'Загрузка',
			maxsize_error: 'Файл �?лишком большой'
             */
...
function getImagesByPath(path,createFolderPath,params,_this){
    var currentDirectory=""
    if(params.subdir!=undefined){
        currentDirectory = params.subdir
    }
    $("#current_path").val("/"+currentDirectory)
    var prevPath=""
    if(currentDirectory.lastIndexOf("/")!=(-1)){  
        prevPath = currentDirectory.substring(0,currentDirectory.lastIndexOf("/"))
    }
    $.post(path,params, function(data) {
        ...
        {                 
            if(!val.isDirectory){
                var img = $('<div class="image-wrapper"><img class="image"src="' + val.thumb + '" rel="' + val.image + '"><div class="text-wrapper">'+val.name+'</div></div>');
                img.click(function() { 
                    redactorActive.imageSetThumb($(this).find('img').attr('rel')); 
                });	  		
                $('#redactor_image_box').append(img);
            }
            else{
                var folderImg = $('<div class="image-wrapper" subdir="'+val.subdir+val.name+'"><div class="folder"></div><div style="padding-top:5px;">'+val.name+'</div></div>');
                folderImg.click(function() {
                    pars={
                        subdir:$(this).attr('subdir')  
                    } 
                    $('#redactor_image_box').html('')
                    getImagesByPath(path,createFolderPath,pars,_this)
                });	  		
                $('#redactor_image_box').append(folderImg);
            }
        });
        
        $("#redactor_toolbar").find("#redactor_toolbar_create_folder").remove()
        var newFolderDiv = $('<div id="redactor_toolbar_create_folder" curPath="'+currentDirectory+'"></div> ');
        newFolderDiv.click(function(){
            pars={
                subdir:$(this).attr('curPath')  
            } 
            createDialog('dialog-form',path,createFolderPath,pars,_this)
        });
        $("#redactor_toolbar").append(newFolderDiv);


        $("#redactor_toolbar").find("#redactor_toolbar_up_folder").remove()
        if(currentDirectory!=""){
            var div = $('<div id="redactor_toolbar_up_folder" prevPath="'+prevPath+'"></div> ');
            div.click(function(){
                pars={
                    subdir:$(this).attr('prevPath')  
                } 
                $('#redactor_image_box').html('')
                getImagesByPath(path,createFolderPath,pars,_this)
            });
            $("#redactor_toolbar").append(div);
        }
        $("#redactor_file_div").find('input').remove()
        $("#redactor_file_div").html('<input type="file" id="redactor_file" name="file" />')
        _this.uploadInit('redactor_file', { auto: true, url: _this.opts.image_upload,currentDir:currentDirectory, trigger: 'redactorUploadBtn', success: function(data) {
                _this.imageUploadCallback(data);
            }.bind2(_this)  });                                
    }, "json");
}

function createDialog(id,path,createFolderPath,pars,_this) {
    $("#" + id).dialog( "destroy" );
    $("#path_for_folder").val(pars.subdir)
    var name = $( "#name" )
    ...
    $( "#dialog-form" ).dialog({
        ...
        buttons: {
            "Create": function() {
                var bValid = true;
                bValid = bValid && checkLength( name, "folder name", 1, 100 );

                if ( bValid ) {
                    sentaDta={
                        folderName:name.val(),
                        folderPath:$("#path_for_folder").val()
                    }
                    $.post(createFolderPath,sentaDta, function(data) {
                        if(data=="true")
                        {
                            $('#redactor_image_box').html('')
                            getImagesByPath(path,createFolderPath,pars,_this)
                            ...
                            updateTips("Folder with name '"+name.val()+"' already exist!" );
                        }
