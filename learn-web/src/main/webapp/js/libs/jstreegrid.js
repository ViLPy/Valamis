        ...
        __init:function () {
            ...
            if (!styled) {
                ...
                styles = [
                    ...
                    '.jstree-grid-separator {display: inline-block; height:20px; ' +
                        'border-width: 0 2px 0 0; *display:inline; *+display:inline; margin-right:0px;float:left;}',
        ...
        _fn:{
            _prepare_headers:function () {
                ...
                for (i = 0; i < cols.length; i++) {
                    ...
                    width -= borPadWidth;
                    margin = i === 0 ? 3 : 0;
                    last = $("<div></div>").css(conf).css({"margin-left":margin, "width":width, "padding":"3px"}).addClass((tr ? "ui-widget-header " : "") + "jstree-grid-header jstree-grid-header-cell jstree-grid-header-" + classAdd + " " + cl).text(val).appendTo(header)
                    ...
                    $(document)
                        .mouseup(function () {
                            ...
                            if (isClickedSep) {
                                ...
                                widths = {};
                                if (!colNum) {
                                    ref.data.grid.treeWidthDiff = currentTree.find("ins:eq(0)").width() + currentTree.find("a:eq(0)").width() - ref.data.grid.columns[0].width;
                                }
                                isClickedSep = false;
                                for (i = 0; i < cols.length; i++) {
                                    widths[cols[i].header] = {w:parseFloat(headers[i].style.width) + borPadWidth, r:i === colNum };
                                }
                ...
                obj.each(function () {
                    ...
                    if (a.length === 1) {
                        ...
                        for (i = 1; i < cols.length; i++) {
                            ...
                            if (s === "attr") {
                                val = col.value && t.attr(col.value) ? t.attr(col.value) : "";
                            } else if (s === "metadata") {
                                val = col.value && t.data(col.value) ? t.data(col.value) : "";
                            }
