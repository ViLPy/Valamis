...
import com.arcusys.valamis.lesson.scorm.model.manifest.{ Manifest, ManifestDocument }
  ...
  def parse = {
    ...
    new ManifestDocument(
      new Manifest(0, manifestVersion, base, scormVersion, defaultOrganizationIdentifier, resourcesBase, title, Some(summary),
        metadata = if (metadataOption.isDefined && (!metadataOption.get.externalMetadataLocations.isEmpty || !metadataOption.get.inlineMetadata.isEmpty)) metadataOption else None, courseId = Some(0), isDefault = false, beginDate = None, endDate = None),
