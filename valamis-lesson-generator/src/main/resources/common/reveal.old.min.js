...
(function (root, factory) {
    ...
    function setupDOM() {
        ...
        dom.foreground = createSingletonNode(dom.wrapper, 'div', 'foregrounds', null);
    ...
    function createForegrounds() {
        ...
        dom.foreground.innerHTML = '';
        dom.foreground.classList.add('no-transition');
        ...
        toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR)).forEach(function (slideh) {
            ...
            if (!printMode) {
                backgroundStack = createForeground(slideh, dom.foreground);
            }

            // Iterate over all vertical slides
            toArray(slideh.querySelectorAll('section')).forEach(function (slidev) {
                if (!printMode) {
                    createForeground(slidev, backgroundStack);
                }
            });

        });

        dom.foreground.style.backgroundImage = '';
        dom.wrapper.classList.remove('has-parallax-foreground');
    }

    /**
     * Creates a foreground for the given slide.
     *
     * @param {HTMLElement} slide
     * @param {HTMLElement} container The element that the background
     * should be appended to
     */
    function createForeground(slide, container) {

        var data = {
            foregroundIframe: slide.getAttribute('data-foreground-iframe')
        };

        var element = document.createElement('div');

        // Carry over custom classes from the slide to the background
        element.className = 'slide-foreground ' + slide.className.replace(/present|past|future/, '');

        // Create a hash for this combination of background settings.
        // This is used to determine when two slide backgrounds are
        // the same.
        if (data.backgroundIframe) {
            element.setAttribute('data-foreground-hash', data.backgroundIframe);
        }

        container.appendChild(element);

        // If backgrounds are being recreated, clear old classes
        slide.classList.remove('has-dark-foreground');
        slide.classList.remove('has-light-foreground');
   ...
   * }), '*' );
     ...
     * all vertical slides (not just the present) will be updated.
     */
    function updateForeground(includeAll) {
        ...
        // Update the classes of all backgrounds to match the
        // states of their slides (past/present/future)
        toArray(dom.foreground.childNodes).forEach(function (foregroundh, h) {

            foregroundh.classList.remove('past');
            foregroundh.classList.remove('present');
            foregroundh.classList.remove('future');

            if (h < indexh) {
                foregroundh.classList.add(horizontalPast);
            }
            else if (h > indexh) {
                foregroundh.classList.add(horizontalFuture);
            }
            else {
                foregroundh.classList.add('present');

                // Store a reference to the current background element
                currentForeground = foregroundh;
            }

            if (includeAll || h === indexh) {
                toArray(foregroundh.querySelectorAll('.slide-foreground')).forEach(function (foregroundv, v) {

                    foregroundv.classList.remove('past');
                    foregroundv.classList.remove('present');
                    foregroundv.classList.remove('future');

                    if (v < indexv) {
                        foregroundv.classList.add('past');
                    }
                    else if (v > indexv) {
                        foregroundv.classList.add('future');
                    }
                    else {
                        foregroundv.classList.add('present');

                        // Only if this is the present horizontal and vertical slide
                        if (h === indexh) currentForeground = foregroundv;
                    }
        ...
        if (previousForeground) {
            ...
            var previousVideo = previousForeground.querySelector('video');
            if (previousVideo) previousVideo.pause();
        ...
        if (currentForeground) {
            ...
            // Start video playback
            var currentVideo = currentForeground.querySelector('video');
            if (currentVideo) currentVideo.play();

            // Don't transition between identical backgrounds. This
            // prevents unwanted flicker.
            var previousForegroundHash = previousForeground ? previousForeground.getAttribute('data-foreground-hash') : null;
            var currentForegroundHash = currentForeground.getAttribute('data-foreground-hash');
            if (currentForegroundHash && currentForegroundHash === previousForegroundHash && currentForeground !== previousForeground) {
                dom.foreground.classList.add('no-transition');
            }

            // hide foregrounds if no exists for current slide
            if (currentForeground.childNodes.length > 0) {
                clearTimeout(foregroundTimeout);
                dom.foreground.style.display = 'block';
            } else {
                foregroundTimeout = setTimeout(function () {
                    dom.foreground.style.display = 'none';
                }, 1000);
            }
        ...
        // bubble it to the .reveal container
        if (currentSlide) {
            ['has-light-foreground', 'has-dark-foreground'].forEach(function (classToBubble) {
                if (currentSlide.classList.contains(classToBubble)) {
                    dom.wrapper.classList.add(classToBubble);
                }
                else {
        ...
        // Allow the first background to apply without transition
        setTimeout(function () {
            dom.foreground.classList.remove('no-transition');
        }, 1);
    ...
    function showSlide(slide) {
        ...
        var foreground = getSlideForeground(indices.h, indices.v);
        if (background) {
        ...
        if (foreground) {
            ...
            var foregroundIframe = slide.getAttribute('data-foreground-iframe');

            if (foreground.hasAttribute('data-loaded') === false) {
                foreground.setAttribute('data-loaded', 'true');
                if (foregroundIframe) {
                    var iframe = document.createElement('iframe');
                    iframe.setAttribute('src', foregroundIframe);
                    iframe.style.width = '100%';
                    iframe.style.height = '100%';
                    iframe.style.maxHeight = '100%';
                    iframe.style.maxWidth = '100%';
     ...
     * index is valid an element will be returned.
     */
    function getSlideForeground(x, y) {

        // When printing to PDF the slide backgrounds are nested
        // inside of the slides
        var horizontalBackground = dom.wrapper.querySelectorAll('.foregrounds>.slide-foreground')[x];
        var verticalBackgrounds = horizontalBackground && horizontalBackground.querySelectorAll('.slide-foreground');
