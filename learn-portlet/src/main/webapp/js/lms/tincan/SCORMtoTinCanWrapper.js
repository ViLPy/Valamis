...
function SetActivity(courseId, courseName, courseDesc, packageID) {
...
//Create the activity
    var activity = new TinCan.Activity({
        id: document.location.protocol + "//" + document.location.host + courseId,
...
function createStatement()
{
	return  new TinCan.Statement({
        ...
        context : {
            registration : registrationId,
            contextActivities:{
                //parent:[{id:"currentPackageID", objectType:"Activity"}],
                grouping:[{id:currentPackageID,objectType:"Activity"}]
            }
        }
}   ,true);
}
...
function SendInteractionData(interactionIndex)
{
    ...
    { // if need to create interaction activity with parent = current activity
        ...
        interactionActivity = new TinCan.Activity({
            id : myTinCan.activity.id + ':interactions#' + activityId,
