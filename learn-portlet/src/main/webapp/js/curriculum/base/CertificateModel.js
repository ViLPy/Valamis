CertificateService = new Backbone.Service({ url: Utils.getContextPath,
        sync: {
            'create': {
                'path': function(){
                    return "/services/certificating?companyID=" + jQuery("#companyID").val();
                },
                 'method' : "post"
            },
            'update': {
...
CertificateCollectionService = new Backbone.Service({ url: Utils.getContextPath,
    sync: {
        'read': function () {
            ...
            return "/services/certificating?companyID=" + jQuery("#companyID").val() + "&page="+ jQuery("#allCertificatesPaginator").pagination('getCurrentPage') +
