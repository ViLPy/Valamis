CertificateService = new Backbone.Service({ url: '/',
  sync: {
    'create': {
      'path': function () {
        return path.api.certificates + '?action=ADD&companyID=' + jQuery('#curriculumCompanyID').val();
      },
      'method': 'post'
    },
    'update': {
      'path': function (model) {
        return path.api.certificates + model.id + '?action=UPDATE&companyID=' + jQuery('#curriculumCompanyID').val();
      },
      'method': 'post'
    },
    'read': {
    ...
    'delete': {
      'path': function (model) {
        return path.api.certificates + model.id + '?action=DELETE';
      },
      'method': 'post'
    }
  },
  ...
  targets: {
    'clone': {
      'path': function (model) {
        return path.api.certificates +  model.id + '?action=CLONE';
      },
      method: 'post'
    },
    'publish': {
      'path': function (model) {
        return path.api.certificates + model.id + '?action=PUBLISH';
      },
      method: 'post'
    },
    'unpublish': {
      'path': function (model) {
        return path.api.certificates + model.id + '?action=UNPUBLISH';
      },
      method: 'post'
    },
    'join' : {
      'path': function (model) {
        return path.api.certificates + model.id + '?action=ADDUSER&userID=' + jQuery('#curriculumStudentID').val();
      },
      method: 'post'
    },
    'leave' : {
      'path': function (model) {
        return path.api.certificates + model.id + '?action=DELETEUSER&userID=' + jQuery('#curriculumStudentID').val();
      },
      method: 'post'
    }
  }
...
CertificateCollectionService = new Backbone.Service({ url: path.root,
  sync: {
    'read': function (e, options) {
      var order = jQuery('#certificateOrder_' + options.portletID).val();
      var sortBy = order.split(':')[0]
      var asc = order.split(':')[1]
      return path.api.certificates +
        '?action=GETALL&companyID=' + jQuery('#curriculumCompanyID').val() +
        '&page=' + options.currentPage +
        '&count=' + options.itemsOnPage +
        '&filter=' + jQuery('#certificateFilter_' + options.portletID).val() +
        '&sortBy=' + sortBy +
  ...
  sync: {
    'read': function (e, options) {
      ...
      return path.api.users
          + jQuery('#curriculumStudentID').val()
          + '/certificates?'
  ...
  sync: {
    'read': function (e, options) {
      ...
      return path.api.users
        ...
        '&available=true' +
        '&isOnlyPublished=true' +
        '&scope=' + jQuery('#certificateScopeFilter_' + options.portletID).val();
    }
  }
});
