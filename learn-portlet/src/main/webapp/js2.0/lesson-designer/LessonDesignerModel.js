var ContentManagerService = new Backbone.Service({  url: path.root,
  sync: {
    'read': function (model) {
      return path.api.quiz + '?action=GETALL' +
...
var ContentManagerModel = Backbone.Model.extend({
  ...
  initialize: function () {
    ...
    this.listenTo(contentManagerEvent, 'lesson:model:changed', function() {this.fetch(); });
