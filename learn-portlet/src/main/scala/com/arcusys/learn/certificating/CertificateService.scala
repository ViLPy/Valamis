...
class CertificateService(configuration: BindingModule) extends ServletBase(configuration) with FileUploadSupport {
  ...
  get("/") {
    ...
    val page = certificateStorage.getPage(companyID, (skip-1)*take, take, filter, sortAZ)
    val result = json(Map(
      "total" -> page.total,
      "currentPage"-> skip,
  ...
  post("/update/:id") {
    ...
    val certificate = new Certificate(id, title, description, "", isPermanent, publishBadge, shortDescription, 0)
