...
object JsonDeserializer {
  ...
  def deserializeStatement(raw: String): Statement = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONDeserializerException("Object with name 'Statement' could not be created from the provided JSON.")
    }
  }
  ...
  def deserializeStatements(raw: String): Seq[Statement] = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONDeserializerException("List of statements could not be created from the provided JSON.")
    }
  }
  ...
  def deserializeStatementResult(raw: String): StatementResult = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONDeserializerException("Object with name 'StatementResult' could not be created from the provided JSON.")
    }
  }
  ...
  def serializeStatementResult(statementResult: StatementResult, formatType: SerializeFormat = SerializeFormat()) = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONSerializerException("JSON raw could not be created from object with name 'StatementResult'.")
    }
  }
  ...
  def serializeStatement(statement: Statement, formatType: SerializeFormat = SerializeFormat()): String = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONSerializerException("JSON raw could not be created from object with name 'Statement'.")
    }
  }
  ...
  def serializeIds(ids: Seq[String]) = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONSerializerException("JSON raw could not be created from list of string.")
    }
  }
  ...
  def deserializeIds(raw: String): Seq[String] = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONDeserializerException("List of string could not be created from the provided JSON.")
    }
  }

  def deserializeAgent(raw: String): Agent = {
    implicit val jsonFormats: Formats = DefaultFormats + new AgentSerializer

    try {
      parse(raw).extract[Agent]
    } catch {
      case e: Exception => throw new JSONDeserializerException("Object with name 'Agent' could not be created from the provided JSON. " + e.getMessage)
    }
  }

  def deserializeActivity(raw: String): Activity = {
    implicit val jsonFormats: Formats = DefaultFormats + new ActivitySerializer

    try {
      parse(raw).extract[Activity]
    } catch {
      case _: Throwable => throw new JSONDeserializerException("Object with name 'Activity' could not be created from the provided JSON.")
    }
  }

  def serializeActivity(activity: Activity): String = {
    implicit val jsonFormats: Formats = DefaultFormats + new ActivitySerializer + new ActorSerializer

    try {
      compact(render(Extraction.decompose(activity)))
    } catch {
      case _: Throwable => throw new JSONSerializerException("JSON raw could not be created from object with name 'Activity'.")
    }
  }
  ...
  def deserializeActor(raw: String): Actor = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONDeserializerException("Object with name 'Actor' could not be created from the provided JSON.")
    }
  }
  ...
  def serializeActor(actor: Actor): String = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONSerializerException("JSON raw could not be created from object with name 'Actor'.")
    }
  }
  ...
  def serializePerson(person: Person): String = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONSerializerException("JSON raw could not be created from object with name 'Person'.")
    }
  }
  ...
  def deserializePerson(raw: String): Person = {
    ...
    try {
      ...
      case _: Throwable => throw new JSONDeserializerException("Object with name 'Person' could not be created from the provided JSON.")
    }
  }
  ...
  class ActorSerializer(formatType: SerializeFormat = SerializeFormat()) extends CustomSerializer[Actor](format => ({
    case jObject: JObject =>
      implicit val jsonFormats: Formats = DefaultFormats + new AgentSerializer(formatType) + new GroupSerializer(formatType)
      jObject.\("objectType").extractOpt[String].getOrElse("Agent") match {
        case "Agent" | "" => jObject.extract[Agent]
        case "Group"      => jObject.extract[Group]
      }
  }, {
    case agent: Agent =>
      implicit val jsonFormats: Formats = DefaultFormats + new AgentSerializer(formatType)
      render(Extraction.decompose(agent))

    case group: Group =>
      implicit val jsonFormats: Formats = DefaultFormats + new GroupSerializer(formatType)
      render(Extraction.decompose(group))
  }))

  class AgentSerializer(formatType: SerializeFormat = SerializeFormat()) extends CustomSerializer[Agent](implicit format => ({
    case jObject: JObject =>
      Agent(
        StatementObjectType.Agent.toString,
        jObject.\("name").extractOpt[String],
        jObject.\("mbox").extractOpt[String],
        jObject.\("mbox_sha1sum").extractOpt[String],
        jObject.\("openid").extractOpt[String],
    ...
    case agent: Agent =>
      if (formatType.Type == "ids") {
        ...
        JObject(JField("objectType", JString(StatementObjectType.Agent.toString)), field)
      } else
        render(Extraction.decompose(agent)(DefaultFormats).removeField(field => field._1.equalsIgnoreCase("storedId")))
  }))

  class GroupSerializer(formatType: SerializeFormat = SerializeFormat()) extends CustomSerializer[Group](format => ({
    case jObject: JObject =>
      implicit val jsonFormats: Formats = DefaultFormats + new AgentSerializer(formatType)

      Group(
        StatementObjectType.Group.toString,
        jObject.\("name").extractOpt[String],
        jObject.\("member").extractOpt[Seq[Agent]],
        jObject.\("mbox").extractOpt[String],
        jObject.\("mbox_sha1sum").extractOpt[String],
        jObject.\("openid").extractOpt[String],
        jObject.\("account").extractOpt[Account],
    ...
    case group: Group =>
      ...
      if (formatType.Type == "ids") {
        ...
        JObject(JField("objectType", JString(StatementObjectType.Group.toString)), field)
      } else
        render(Extraction.decompose(group)(DefaultFormats).removeField(field => field._1.equalsIgnoreCase("storedId")))
  ...
  class ActivitySerializer() extends CustomSerializer[Activity](implicit format => ({
    case jValue: JValue => //jObject.extract[Result]
      Activity(
        jValue.\("objectType").extract[String],
        jValue.\("id").extract[String],
        jValue.\("name").extractOpt[LanguageMap],
        jValue.\("description").extractOpt[LanguageMap],
        jValue.\("theType").extractOpt[String],
        jValue.\("moreInfo").extractOpt[String],
        jValue.\("interactionType").extractOpt[String].map(InteractionType.withName),
        jValue.\("correctResponsesPattern").extract[Set[String]],
        jValue.\("choices").extract[Seq[InteractionComponent]],
        jValue.\("scale").extract[Seq[InteractionComponent]],
        jValue.\("source").extract[Seq[InteractionComponent]],
        jValue.\("target").extract[Seq[InteractionComponent]],
        jValue.\("steps").extract[Seq[InteractionComponent]],
        jValue.\("extensions").extractOpt[Map[String, String]],
        jValue.\("storedId").extractOpt[Int]
      )
  }, {
    case result: Activity => render(Extraction.decompose(result)(DefaultFormats))
  ...
  class VerbSerializer() extends CustomSerializer[Verb](implicit format => ({
    case jValue: JValue => //jObject.extract[Result]
      Verb(
        jValue.\("id").extract[String],
        jValue.\("display").extractOpt[LanguageMap].getOrElse(Map[String, String]())
      )
  }, {
    case verb: Verb => render(Extraction.decompose(verb)(DefaultFormats))
    ...
    case activity: Activity =>
      if (formatType.Type == "ids") {
        ...
        if (formatType.Type == "canonical" && formatType.Lang != "") {
          ...
          name = name.filter(l1 => langs.exists(l2 => l2.contains(l1._1))).take(1)
          description = description.filter(l1 => langs.exists(l2 => l2.contains(l1._1))).take(1)
        ...
        val definition = JObject(
          ...
          JField("moreInfo", JString(activity.moreInfo.orNull)),
  ...
  class StatementSerializer(formatType: SerializeFormat = SerializeFormat()) extends CustomSerializer[Statement](format => ({
    case jValue: JValue =>
      implicit val jsonFormats: Formats = DefaultFormats + JsonSupport.extFormats.uuid + new ActorSerializer(formatType) + new StatementObjectSerializer + new InteractionTypeSerializer + new ResultSerializer + new ScoreSerializer + new VerbSerializer
      Statement(
        jValue.\("id").extractOpt[UUID].orNull,
