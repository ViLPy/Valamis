...
class JsonSerializerTest {
  ...
  @Test
  def AgentSerializerTest() {

    val expected = Agent("Agent", None, None, None, None, Some(Account("http://www.example.com", "1625378")))
    val actualJson = JsonDeserializer.serializeActor(expected)
    val actual = JsonDeserializer.deserializeActor(actualJson)
  ...
  @Test
  def GroupSerializerTest() {

    val expected = Group("Group", None, None, None, None, None, Some(Account("http://www.example.com", "1625378")))
    val actualJson = JsonDeserializer.serializeActor(expected)
  ...
  def StatementTest() {
    ...
    val statement = new Statement(
      UUID.fromString("e59a54c1-f66b-43b8-bf61-d9b511a9e869"),
      Agent("Agent", None, None, None, None, Some(Account("http://www.example.com", "1625378"))),
      Verb("http://adlnet.gov/expapi/verbs/attempted", Map("und" -> "attempted")),
      new Activity("Activity", "http://tincanapi.com/Course/NashvilleMuseumsTour",
        Some(Map("en-US" -> "Golf Example - Tin Can Course")),
        Some(Map("en-US" -> "An exploration of museums in the greater Nashville TN area.")),
        Some("http://adlnet.gov/expapi/activities/course"), None, None, Set(), Seq(), Seq(), Seq(), Seq(), Seq(), Seq()),
      None, None,
      None,
      None, None, None, Seq()
    )

    JsonDeserializer.serializeStatement(statement)
    val serialized = JsonDeserializer.serializeStatement(statement)

    //assertFalse(serialized=="{}")

    System.out.println(serialized)
    val deserialized = JsonDeserializer.deserializeStatement(serialized)
  ...
  def StatementTest2() {
    ...
    val statement = new Statement(
      ...
      Verb("http://adlnet.gov/expapi/verbs/attempted", Map("und" -> "attempted")),
      new Activity("Activity", "http://tincanapi.com/Course/NashvilleMuseumsTour",
        Some(Map("en-US" -> "Golf Example - Tin Can Course","ru-RU" -> "Golf Example - Tin Can Course")),
        Some(Map("en-US" -> "An exploration of museums in the greater Nashville TN area.","ru-RU" -> "Golf Example - Tin Can Course")),
        Some("http://adlnet.gov/expapi/activities/course"), None, None, Set(), Seq(), Seq(), Seq(), Seq(), Seq(), Seq()),
      None, None,
      None,
      None, None, None, Seq()
    ...
    //exact
    JsonDeserializer.serializeStatement(statement)
    var serialized = JsonDeserializer.serializeStatement(statement)


    System.out.println(serialized)
    var deserialized = JsonDeserializer.deserializeStatement(serialized)
    assertEquals(statement, deserialized)

    //ids
    JsonDeserializer.serializeStatement(statement)
    serialized = JsonDeserializer.serializeStatement(statement,SerializeFormat("ids"))


    System.out.println(serialized)
    deserialized = JsonDeserializer.deserializeStatement(serialized)
    //assertEquals(statement, deserialized)

    //canonical
    JsonDeserializer.serializeStatement(statement)
    serialized = JsonDeserializer.serializeStatement(statement,SerializeFormat("canonical","en-US"))


    System.out.println(serialized)
    deserialized = JsonDeserializer.deserializeStatement(serialized)
    //assertEquals(statement, deserialized)
    assertEquals(deserialized.obj.asInstanceOf[Activity].name.size,1)
    assertEquals(deserialized.obj.asInstanceOf[Activity].description.size,1)


  }

  @Test
  def StatementResultTest() {
    ...
    val statement = new Statement(
      ...
      new Activity("Activity", "http://tincanapi.com/Course/NashvilleMuseumsTour",
        ...
        Some("http://adlnet.gov/expapi/activities/course"), None, None, Set(), Seq(), Seq(), Seq(), Seq(), Seq(), Seq()),
      None, None,
      Some(new DateTime("2013-10-18T10:07:42.000Z").toDate),
      None, None, None, Seq()
    )

    val statement1 = new Statement(
      UUID.fromString("e59a54c1-f66b-43b8-bf61-d9b511a9e868"),
      Agent("Agent", None, None, None, None, Some(Account("http://www.example.com", "1625378"))),
      Verb("http://adlnet.gov/expapi/verbs/attempted", Map("und" -> "attempted")),
      SubStatement(Agent("Agent", None, None, None, None, Some(Account("http://www.example.com", "1625378"))),
        Verb("http://adlnet.gov/expapi/verbs/attempted", Map("und" -> "attempted")),
        new Activity("Activity", "http://tincanapi.com/Course/NashvilleMuseumsTour",
          Some(Map("en-US" -> "Golf Example - Tin Can Course")),
          Some(Map("en-US" -> "An exploration of museums in the greater Nashville TN area.")),
          Some("http://adlnet.gov/expapi/activities/course"), None, None, Set(), Seq(), Seq(), Seq(), Seq(), Seq(), Seq(), None),
        "SubStatement"),
      None, None,
      Some(new DateTime("2013-10-18T10:07:42.000Z").toDate),
      None, None, None, Seq()
    ...
    val statement2 = new Statement(
      UUID.fromString("e59a54c1-f66b-43b8-bf61-d9b511a9e867"),
      Agent("Agent", None, None, None, None, Some(Account("http://www.example.com", "1625378"))),
      Verb("http://adlnet.gov/expapi/verbs/attempted", Map("und" -> "attempted")),
      StatementReference(UUID.fromString("e59a54c1-f66b-43b8-bf61-d9b511a9e869"),
        "StatementRef"),
      None, None,
      Some(new DateTime("2013-10-18T10:07:42.000Z").toDate),
      None, None, None, Seq()
    ...
    val statement3 = new Statement(
      UUID.fromString("e59a54c1-f66b-43b8-bf61-d9b511a9e867"),
      Agent("Agent", None, None, None, None, Some(Account("http://www.example.com", "1625378"))),
      Verb("http://adlnet.gov/expapi/verbs/attempted", Map("und" -> "attempted")),
      Agent("Agent", Some("TestUser"), None, None, None, None),
      None, None,
      Some(new DateTime("2013-10-18T10:07:42.000Z").toDate),
      None, None, None, Seq()
    )

    val statementResult = StatementResult(Seq(statement, statement1, statement2, statement3), "")

    //exact
    var serialized = JsonDeserializer.serializeStatementResult(statementResult)

    System.out.println(serialized)

    var deserialized = JsonDeserializer.deserializeStatementResult(serialized)
    assertEquals(statementResult.statements.size, deserialized.statements.size)
    assertEquals(statementResult.statements.head.id, deserialized.statements.head.id)

    // ids
     serialized = JsonDeserializer.serializeStatementResult(statementResult,SerializeFormat("ids"))

    System.out.println(serialized)

    // deserialized = JsonDeserializer.deserializeStatementResult(serialized)
    //assertEquals(statementResult.statements.size, deserialized.statements.size)
    //assertEquals(statementResult.statements.head.id, deserialized.statements.head.id)

    //canonical
     serialized = JsonDeserializer.serializeStatementResult(statementResult,SerializeFormat("canonical","en-US"))
