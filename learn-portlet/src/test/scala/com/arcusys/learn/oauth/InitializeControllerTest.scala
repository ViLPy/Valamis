...
class InitializeControllerTest extends ScalatraFlatSpec with MockFactory with ProxyMockFactory with MockInjectableFactory {
  ...
  bindingModule.modifyBindings(module => {
    ...
    addServlet(new InitializeController(module), "/*")
  })

  "InitializeController" should "take fetch token" in {
