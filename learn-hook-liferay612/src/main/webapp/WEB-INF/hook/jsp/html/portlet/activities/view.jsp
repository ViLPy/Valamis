...
if (group.isUser()){
    ...
    ArrayList<SocialActivity> activitiesWithCertificateData = new ArrayList<SocialActivity>();
    for(SocialActivity e:activities){
        activitiesWithCertificateData.add(e);
    }

    String className = "com.arcusys.learn.scorm.tracking.model.certificating.Certificate";

    for(SocialActivity e:SocialActivityLocalServiceUtil.getActivities(className, 0, max)){
        activitiesWithCertificateData.add(e);
    }

    Collections.sort(activitiesWithCertificateData);

    int counter = 0;
    int size = activitiesWithCertificateData.size();
    while( counter <  max && size > counter) {
        result.add(activitiesWithCertificateData.get(counter));
        counter++;
    }
}
