...
class QuestionService extends ScalatraServlet
{
  ...
  post("/SetAnswers") {
    val id = params.getOrElse("questionID", halt(405, "ID is not specified")).toInt
    val questionType = params.getOrElse("questionType", halt(405, "Question type is not specified")).toInt
    val answersJSON = params.getOrElse("answerData", halt(405, "Answer data is not specified"))
    ...
    val question = getQuestionStorage.getByID(id).get
    ...
    (0 until answersSeq.length).map(i => getAnswerStorage.create(id, i, answersSeq.apply(i)))
  }
  ...
  post("/") {
    ...
    getQuestionStorage.createQuestion(entity)._1.toString
  }
  ...
  post("/Update") {
    ...
    getQuestionStorage.modifyQuestion(entity)._1.toString
  }
