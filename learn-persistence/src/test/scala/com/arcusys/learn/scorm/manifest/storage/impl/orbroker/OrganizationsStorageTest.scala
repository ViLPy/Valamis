...
class OrganizationsStorageTest(dbFileName: String) extends ParameterizedUnitTests(dbFileName){
  ...
  @Test
  def canGetByPackage() {
    val testPackage1Id = packagesStorage.createAndGetID(new Manifest(12, None, None, "", Some("defaultOrganizationIdentifier"), Some("resourcesBase/"), "title",courseID= Some(0) , isDefault = false))
    val testPackage2Id = packagesStorage.createAndGetID(new Manifest(122, None, None, "", Some("defaultOrganizationIdentifier2"), Some("resourcesBase2/"), "title2",courseID= Some(0), isDefault = false ))
